# Process Holter V1.2 - Utility for processing Holter signals
#
...
def get_data(pathname, filename):
  ...
  ecg_name, axi = ('ECG_1', 3) if 'ECG_1' in sample_rate else ('ECG', 1)
      
  acc_timestamp = [start_date + datetime.timedelta
                   (microseconds=1./sample_rate['Accelerometer_X']*n*1e6)
                   for n in range(len(signals[axi]))]  # 1 is index of Accel_X
  
  ecg_timestamp = [start_date +
                   datetime.timedelta(microseconds=1./sample_rate[ecg_name]*n*1e6)
                   for n in range(len(signals[0]))]  # 0 is index of ECG
  acc = pd.DataFrame(np.transpose(signals[axi:axi+3]), index=acc_timestamp,
              columns=[header['label'] for header in signal_headers[axi:axi+3]])
  ...
  acc.dropna()
  return ecg, acc, sample_rate, ecg_name
...
def spectrum(sig, sample_rate):
  ...
  sig_cond = butter_bandpass_filter(sig_norm, 6.67/60, 40/60, sample_rate,
                                    order=2)
...
def calculate_rr(acc, sample_rate, window_length, calc_every):
  ...
  for i in range(0, len(acc_tot)-rr_win, 
    ...
    plt.plot(freq*60, spec, alpha = 0.2)
...
def main():
  pathname = '{**INSERT PATHNAME OF FOLDER HERE**}'  # <-- * 
